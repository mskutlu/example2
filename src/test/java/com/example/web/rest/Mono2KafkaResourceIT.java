...
class Mono2KafkaResourceIT {
    ...
    void setup() {
        ...
        Mono2KafkaResource kafkaResource = new Mono2KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/mono-2-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/mono-2-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
